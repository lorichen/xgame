...
char * GetMD5Hash(char szHashOut[33],const unsigned char *pBufIn, DWORD nInSize)
{
	MD5_CTX m_md5;
...
unsigned char * GetMd5HashHex(char* buf  ,DWORD  buflen,unsigned char* outputmd5)
{	
	MD5_CTX context;

	MD5Init (&context);
	MD5Update (&context, (unsigned char*)buf, buflen);
	MD5Final (&context);
