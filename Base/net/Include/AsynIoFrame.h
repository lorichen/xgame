	...
	*/
	struct AsynIoResult : public OVERLAPPED
	{
		...
		*/
		...
		@purpose   : 准备异步IO要用的缓冲区
		@param len : 希望的缓冲区长度
		@param pRecieved : 上一次接收到的数据包不完整，已接收到的数据
		@param dwRecievedLen : 上一次接收到的数据包不完整，已接收到的数据长度
		@param dwRestLen : 上一次接收到的数据包不完整，剩下的数据长度
		@warning   : 如果之前曾经调用PrepareBuffer准备了缓冲区,则再次调用时会把先前的缓冲区释放掉并重新分配
		*/
		virtual	void PrepareBuffer( unsigned long len, LPVOID pRecieved, DWORD dwRecievedLen, DWORD dwRestLen) = 0;

		//crr add，上一次接收到的数据包不完整，返回已接收到的数据长度
		...
		*/
		...
		/**
		@purpose    :通常用于读文件时设置文件偏移值
		*/
		virtual ULONG64 GetOffsetBytes() = 0;

		/**
		@purpose     :通常用于读文件时设置文件偏移值
		@param offset:读写文件时的偏移值
		*/
...
#	define	MAX_ASYN_IO_BUFF_COUNT      16
	...
	/**
	@name             : 初始化异步IO框架
	@param dwThreadNum: 使用多少个线程去处理异步IO
	@return		      : 1 ... OK
						-1... 初始化前摄器失败
						-2... 启动工作线程池失败
						-3... 初始化网络失败
