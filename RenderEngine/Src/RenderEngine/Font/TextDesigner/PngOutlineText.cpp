...
{
	m_pBkgdBitmap = new Gdiplus::Bitmap(800, 600, PixelFormat32bppARGB);
	...
	Gdiplus::Color fontcolor)
{
	...
	//没有任何字体效果的情况下
	if(m_pTextStrategy==NULL&&m_bEnableShadow==false)
	{
		using namespace Gdiplus;
		GraphicsPath path;
	...
	//by yhc
		if(m_pTextStrategy==NULL)
		{
			...
			*pfDestHeight= ptDraw.Y;
			GDIPath::MeasureGraphicsPath(pGraphics, &path, pfDestWidth, pfDestHeight);
			return true;
		}
		else
		{
