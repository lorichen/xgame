...
FTFace::FTFace( const uchar*pBufferBytes, size_t bufferSizeInBytes)
:   numGlyphs(0),
    err(0)
{
    const FT_Long DEFAULT_FACE_INDEX = 0;
...
bool FTFace::Attach( const uchar*pBufferBytes, size_t bufferSizeInBytes)
{
    FT_Open_Args open;
...
const FTSize& FTFace::Size( const uint size, const uint res)
{
...
FTPoint FTFace::KernAdvance( uint index1, uint index2)
{
    float x, y;
    x = y = 0.0f;
...
FT_GlyphSlot FTFace::Glyph( uint index, FT_Int load_flags)
{
